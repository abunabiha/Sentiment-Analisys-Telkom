# -*- coding: utf-8 -*-
"""Sentiment Analysis Aplikasi Telkomsel-Mobile-SVM.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vLzLS30vS-aCaXbo25vJuIrHSdhbavs5
...
Pada proyek ini, kita akan melakukan analisis sentimen terhadap ulasan dan pendapat pengguna terkait aplikasi "Telkomsel Mobile" di Play Store. Analisis sentimen akan membantu kita untuk memahami bagaimana pengguna merasakan dan mengungkapkan pandangan mereka terhadap aplikasi ini.

Dengan menggunakan berbagai teknik pemrosesan teks dan algoritma machine learning, kita akan mencoba mengidentifikasi apakah ulasan pengguna terhadap aplikasi "Telkomsel Mobile" cenderung positif, negatif, atau netral. Hasil analisis sentimen ini dapat memberikan wawasan berharga kepada pengembang aplikasi untuk meningkatkan pengalaman pengguna. Mari kita mulai menjelajahi ulasan pengguna dan menganalisis sentimen di sekitar aplikasi "Telkomsel Mobile" dengan lebih mendalam.
"""
...
with open('Data_Review_Telkom.csv', mode='w', newline='', encoding='utf-8') as file:
...
app_reviews_df.head()
app_reviews_df.to_csv('Data_Review_Telkom.csv', index=False)
...
# Buat prediksi pada set pengujian dan pelatihan
y_pred = clf.predict(X_test)
y_train_pred = clf.predict(X_train) # Calculate predictions on training set
...
"""Dalam kasus Sentiment Analysis ini, SVM mencapai akurasi tertinggi sebesar 85,43%.
...
# Buat objek Naive Bayes (Multinomial karena data teks)
clf = MultinomialNB()
...
clf = RandomForestClassifier(n_estimators=100, random_state=42)  # n_estimators adalah jumlah pohon di hutan
...
clf = GradientBoostingClassifier(n_estimators=100, random_state=42)  # n_estimators adalah jumlah pohon
...
print(f"\nModel dengan akurasi tertinggi: {best_model} ({results[best_model]:.4f})")

"""# Pengukuran performa evaluasi model binary classification menggunakan akurasi (accuracy), presisi (precision), recall, F1-score, dan Area Under the Receiver Operating Characteristic Curve (AUROC)."""
...
for name, model in models.items():
    ...
    accuracy = accuracy_score(y_test, y_pred)
    precision = precision_score(y_test, y_pred, average='weighted')  # Menggunakan 'weighted' untuk menangani multiclass
    recall = recall_score(y_test, y_pred, average='weighted')
    f1 = f1_score(y_test, y_pred, average='weighted')

    # Hitung AUROC jika model mendukung predict_proba
    if hasattr(model, "predict_proba"):
        y_prob = model.predict_proba(X_test)[:, 1]  # Ambil probabilitas kelas positif
        auroc = roc_auc_score(y_test, y_prob)
    else:
        auroc = "N/A"

    results[name] = [accuracy, precision, recall, f1, auroc]
...
"""# Uji coba hasil Prediksi

Untuk memudahkan bisa menggunaka daftar katai ini atau sendiri

Komentar Positif:
"Aplikasi Telkomsel sangat mudah digunakan dan memiliki tampilan yang menarik."
"Layanan pelanggan di aplikasi ini sangat responsif dan membantu sekali."
"Aplikasi ini memudahkan saya untuk cek kuota dan pulsa dengan cepat."
"Sinyal Telkomsel selalu stabil, bahkan di daerah terpencil sekalipun."
"Fitur MyTelkomsel Points sangat bermanfaat untuk mendapatkan diskon menarik."
Komentar Negatif:
"Aplikasi sering kali lambat dan kadang tidak bisa dibuka sama sekali."
"Saya sering mengalami masalah dengan pembayaran yang gagal di aplikasi."
"Menu navigasi dalam aplikasi ini sangat membingungkan dan kurang intuitif."
"Telkomsel sering tiba-tiba menghabiskan kuota tanpa ada pemberitahuan terlebih dahulu."
"Notifikasi dari aplikasi ini terlalu sering dan cukup mengganggu."
"""
...
# Assuming you have a trained logistic regression model named 'clf' from a previous step
# If it's named something else, replace 'clf' with the correct variable name
prediksi_sentimen = clf.predict(X_kalimat_baru)
